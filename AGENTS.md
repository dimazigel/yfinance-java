...
- **Nullness:** every package is JSpecify `@NullMarked` (see each `package-info.java`). Unannotated = never null; mark anything Yahoo may omit `@Nullable`. DTO components are all `@Nullable` by design. Record accessors inherit component nullness, so a component normalised in a compact constructor (lists, `events`) stays non-null in its declaration. Prefer restructuring code (locals, a small helper) over `@SuppressWarnings("NullAway")`. Model leaf values stay `@Nullable` (Yahoo omits them per instrument); the caller-side escape hatch is `Required.value(...)` / the `require(accessor, "field")` sugar on `Quote`, `Info`, `PriceBar`, `HistoryMetadata`, which throws `YFMissingDataException` — never coerce a missing value to a default.
- **Logging:** SLF4J (`LoggerFactory.getLogger(X.class)`); `slf4j-api` is an `api` dependency and the only logging dependency — never add a backend to main. Tests bind Logback at WARN (`src/test/resources/logback-test.xml`) — not slf4j-simple, whose MDC is a no-op. Use the fluent API (`LOG.atWarn().log(...)`, `LOG.atLevel(level)...`). `INFO` for rare state changes worth an operator's attention, `WARN` for degraded behaviour, `DEBUG` for per-request detail; `{}` placeholders, never concatenation. **Context:** every service entry point wraps its work in `LogContext.scope(op, symbol)` (MDC keys `yf.op`, `yf.symbol`; the `LogContextInterceptor` adds `yf.endpoint` per HTTP call). Reuse the existing `op` names; add a new one only for a genuinely new operation. Event facts (`status`, `attempt`, `delayMs`, `reason`, `cause`) go as `addKeyValue(...)`, not into MDC. **Level discipline:** `WARN` only for degraded-or-gave-up (empty in a healthy run); `INFO` once per client lifetime or per batch, never per symbol; `DEBUG` once per request (`RequestLogInterceptor`) or per dropped datum — every lenient `null`/skip in `MapperSupport`/mappers logs what it dropped and why. Never log an exception the library also throws. Assert log output in tests with `testsupport.LogCapture`.
