...
Unit rules: `changePercent`, v7 `dividendYield`, `fiveYearAvgDividendYield`, `debtToEquity`, `postMarketChangePercent`, v7 `netExpenseRatio`, v7 `ytdReturn`, v7 `trailingThreeMonthReturns` and v7 `trailingThreeMonthNavReturns` are PERCENTS on the wire → stored as fractions; all other yields/margins/held-percent are already fractions; epoch seconds/millis → `Instant`; date-only epochs → `LocalDate`. The fund-snapshot v7 percent fields were verified on live captures (SPY/GLD/VFIAX/CSPX.L): v7 expense ratio 0.0945/0.4/0.04/0.07 vs the matching quoteSummary fraction 0.000945/0.004/0.00040/0.00070; v7 `dividendYield` 0.98 vs `summaryDetail.yield` 0.0098; v7 `ytdReturn` 13.07293 vs `fundPerformance.trailingReturns.ytd` 0.1307293; v7 `trailingThreeMonthReturns` 1.65706 vs 0.0165706 — all ten-to-the-second-power percent-vs-fraction pairs, confirming the wire truly serves a percent on v7 for these four keys. `OptionContract.changePercent` (wire `percentChange`) is likewise a percent on the wire, stored as a fraction. Any field whose unit is marked *verify at impl* must be confirmed against a fixture before the mapper is written.
...
| `currentDividend.yield` | C:currentDividend | `BigDecimal` | `v7:dividendYield` → `qs:summaryDetail.dividendYield` | 59% | v7 PERCENT (verified: SPY 0.98 vs summaryDetail 0.0098, Task 7), summaryDetail already a fraction → fraction |
...
| `statistics.shortInterest.date` | C:shortInterest | `LocalDate` | `qs:defaultKeyStatistics.dateShortInterest` | 75% | epoch s → LocalDate |
...
| `statistics.lastSplit.date` | C:lastSplit | `LocalDate` | `qs:defaultKeyStatistics.lastSplitDate` | 64% | perfect cluster 63.6% |
...
| `statistics.lastDividend.date` | C:lastDividend | `LocalDate` | `qs:defaultKeyStatistics.lastDividendDate` | 62% |  |
...
| `expenseRatio` | O | `BigDecimal` | `v7:netExpenseRatio` → `qs:fundProfile.feesExpensesInvestment.annualReportExpenseRatio` | 89% | 89%: some listings lack it (was "UCITS lack it everywhere"; CSPX.L, previously a documented miss, now reports an expense ratio in both sources); unit verified in Task 7 (see header) |
| `yield` | O | `BigDecimal` | `v7:dividendYield` → `qs:summaryDetail.yield` → `qs:defaultKeyStatistics.yield` | 82% | 82%; v7 PERCENT verified against summaryDetail fraction (Task 7, see header) |
| `navPrice` | O | `BigDecimal` | `qs:summaryDetail.navPrice` | 92% | 92% |
| `beta3Year` | O | `BigDecimal` | `qs:defaultKeyStatistics.beta3Year` | 82% | 82% |
| `ytdReturn` | R | `BigDecimal` | `v7:ytdReturn` → `qs:defaultKeyStatistics.ytdReturn` → `qs:fundPerformance.trailingReturns.ytd` | 100% | 100% via fundPerformance; v7 PERCENT (see header) |
| `threeMonthReturn` | R | `BigDecimal` | `v7:trailingThreeMonthReturns` → `qs:fundPerformance.trailingReturns.threeMonth` | 100% | 100%; v7 PERCENT (see header) |
| `trailingThreeMonthNavReturns` | O | `BigDecimal` | `v7:trailingThreeMonthNavReturns` | 82% | 82%; v7 PERCENT (see header) |
...
| `expenseRatio` | R | `BigDecimal` | `v7:netExpenseRatio` → `qs:fundProfile.feesExpensesInvestment.annualReportExpenseRatio` → `qs:defaultKeyStatistics.annualReportExpenseRatio` | 100% | v7 PERCENT (see header) |
...
| `ytdReturn` | R | `BigDecimal` | `v7:ytdReturn` → `qs:summaryDetail.ytdReturn` → `qs:fundPerformance.trailingReturns.ytd` | 100% | v7 PERCENT (see header) |
| `threeMonthReturn` | R | `BigDecimal` | `v7:trailingThreeMonthReturns` → `qs:fundPerformance.trailingReturns.threeMonth` | 100% | v7 PERCENT (see header) |
...
| `startDate` | R | `LocalDate` | `qs:assetProfile.startDate` | 100% | ISO date string (fixture value `"2010-07-13"`); ruled Task 18 — distinct from the *snapshot* `startDate` above (`v7:startDate` → `qs:summaryDetail.startDate`, an epoch value) |
